...
- **Issue #159 / PR #160 — device-first Local Connector correction:** **CLOSED / ACCEPTED / MERGED**；ordinary Direct Local edit/filesystem/Excel/DOCX/PDF/workspace-effect operations use connected Local Connector/device endpoint + workspace/resource scope + operation-specific safety, without task/Parent/execution authority tokens. Governance remains optional orchestration/continuity/acceptance overlay；resource containment、canonical/sensitive-path policy、primary-vs-secondary access、MutationOwner、stale-write/baseSha256 and atomic/readback protections remain.
- **Issue #161 / PR #162 — device-authorized process / PowerShell capability:** **CLOSED / ACCEPTED / MERGED**；public `process_start/process_read_output/process_terminate` use opaque process handles and ordinary device/workspace context, with truthful OS-user-level reach and no Governance dependency. Real product evidence later proved successful Windows PowerShell execution through the current Local Connector.
- **Issue #167 — Plus MCP admission repeatability:** **CLOSED / MIXED PRODUCT-ADMISSION EVIDENCE**；the identical harmless `process_start` call was pre-dispatch blocked in one fresh conversation and reached Local Connector in another. This falsifies a simple static “Plus/process_start always blocked” rule. The planned `process_exec(executable,args[])` Phase B was not authorized.
- **Issue #168 — Windows process backend diagnostic:** **CLOSED / PASS**；`git_status` and public `process_start(shell="powershell")` both reached Local Connector；PowerShell completed with `exitCode=0` and the exact sentinel. The prior single `CHILD_START_FAILED` was not reproduced and is not evidence of a permanent backend defect. Current operating mode is real-workload adoption: Local Connector first when admitted；other providers such as Remote Desktop Commander are fallback capacity.
- **Issues #176 / #177 / #178 / #181 / #183 / #185 — multi-device Local Connector:** **ACCEPTED / PROVEN**；device-first authority is `ChatGPT account -> authorized devices -> per-device local policy -> workspace/resource context -> operation`. The accepted account-facing path is `ChatGPT -> one Local Connector Relay -> exact paired device -> device-local Stable Runtime/Local MCP`; #185 proved the real one-connector/two-physical-device path on current `main@6d7540086d08f430e5e64b7abc411c64627a92f1`, including exact-device routing, executor-aware Ready, handle affinity/fencing, reconnect continuity, runtime-restart fencing, non-ready no-failover, revoke isolation and device-local filesystem denial. This does **not** authorize production relay hosting, final OAuth/OIDC binding, default flip, release, or retirement of the canonical single-device Secure Tunnel path.
...
| Device-first Direct Local decoupling | **CLOSED / ACCEPTED** | Issue #159 / PR #160；ordinary local access no longer depends on Governance mission authority |
| Device-authorized process / PowerShell | **CLOSED / ACCEPTED** | Issue #161 / PR #162；typed process lifecycle with opaque handles；real Windows PowerShell execution later PASS |
| Plus MCP admission repeatability | **CLOSED / MIXED EVIDENCE** | Issue #167；identical harmless process call both pre-dispatch blocked and dispatched across fresh chats；no wrapper rewrite authorized |
| Windows process backend diagnostic | **CLOSED / PASS** | Issue #168；`git_status` + public PowerShell process completed successfully；backend healthy |
| Multi-device Local Connector product path | **CLOSED / ACCEPTED / PROVEN** | Issues #176/#177/#178/#181/#183/#185；one account-facing Relay -> exact paired device -> device-local Stable Runtime/Local MCP；real two-device dogfood PASS；no production/default/release implication |

## Post-v0.2 Local Connector / DesktopCommander recovery baseline — ACCEPTED through #128

- **Composite Local execution — [#116](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/116) / [#117](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/117) / [#118](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/118) / [#120](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/120):** Parent accepted `COMPOSITE_DESKTOPCOMMANDER_ENGINE`. The existing approved custom App + Secure MCP Tunnel terminates at a thin Orchestrator HTTP MCP façade; bounded commodity `read` / `search` / fixed `git_status` / `git_diff` execution is delegated to exact-pinned `@wonderwhy-er/desktop-commander@0.2.51` as a local child MCP over stdio. Hosted Remote Desktop Commander is **not** the execution dependency, and the Codex App Server adapter remains a separate sustained-execution path. #120 removed superseded bespoke direct read/search/git execution fallbacks while retaining evidenced parent-side resource/security/normalization checks plus bounded edit, Governance, Codex and recovery semantics. No generic public DesktopCommander shell/process/config/write-file surface was introduced.
- **Product naming — [#119](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/119):** the human-facing custom App/display name is **Local Connector**, with description `Connect ChatGPT to authorized files, workspaces, and local execution capabilities on your computer.` Repository/server/programmatic identity remains `chatgpt-codex-orchestrator`; historical human-facing `mcp-plus-web-probe` is retired; DesktopCommander is dependency/provenance, not product branding.
- **Dependency continuity — [#122](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/122):** the exact DesktopCommander 0.2.51 npm artifact and exact upstream source commit `092ce0b841e86455f12e41f4dc36399a7522ecb5` are escrowed under the project-owned [dependency-escrow prerelease](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/releases/tag/dependency-escrow-desktop-commander-0.2.51), with the repository pointer in [`dependency-escrow/desktop-commander-0.2.51.json`](dependency-escrow/desktop-commander-0.2.51.json). Project-owned retrieval plus bounded cold recovery without live DesktopCommander npm/GitHub source was proven against the accepted child semantic contract. This escrow is supply continuity only; it is not an Orchestrator product release, operational-default change, fork or mirror service.
- **Resource authority — [#123](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/123) / [#124](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/124) / [#127](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/127):** reject `PRIMARY_WORKSPACE == COMPLETE_LOCAL_RESOURCE_AUTHORITY`. The primary workspace is the primary task context + default mutation root; the Host trust ceiling is the coarse Human/admin maximum; explicit mission/session-scoped secondary file/root read grants provide auxiliary read/search authority. Those grants are bound through read-only `workspace_open.secondaryReadGrants` handles, need no Governance mutation and require no per-mission Stable Runtime trust-root restart/reconfiguration. External write remains unauthorized. Direct Local edit, Codex writable roots, process/shell, network, MutationOwner, Repository Identity Fence and Parent authority remain unchanged. Real ChatGPT custom-App synthetic dogfood passed #127.
- **Governance Product blocker — [#111](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/111):** the dedicated low-risk PLAN-only `governance_plan` Orchestrator implementation is complete. The current observed downstream blocker is an **upstream ChatGPT Developer-MCP product safety pre-check before Local Governance execution**. Current evidence does not justify repeated probing, reclassification, weaker Governance, or a bypass. #111 completion means the Orchestrator implementation is complete; it does **not** mean downstream China Demand E2 is complete.
- **Security residual — [#128](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/128):** the current production audit residual remains **2 high + 2 moderate package objects** through DesktopCommander transitives; the underlying current advisory paths are `sharp` and `uuid`. Parent accepted the relevant advisories as `NOT_REACHABLE_BY_EVIDENCE` for the **current accepted Local Connector surface**, so current Local Connector usage need not pause. Keep the exact 0.2.51 pin; do not add speculative overrides/forks/replacements. Re-evaluate if upstream publishes a real fixing candidate or if Orchestrator expands into currently excluded special-format or generic DesktopCommander surfaces. This is surface-specific and is **not** a claim that DesktopCommander 0.2.51 is generally vulnerability-free.

GitHub current code/Issues/PRs/CI/tag/Release remain authoritative; this section is a concise recovery summary, not a second control surface and not a new v0.2.1/v0.3 milestone.

## Device-first Local Connector / multi-device routing — ACCEPTED through #185

- **#176 — local filesystem authority:** each device keeps an explicit local-human-owned policy: legacy/default `selected_roots` or explicit `os_user_scope`. `workspaceId` remains a resource/cwd context in both modes；no public MCP/Relay call can widen the device-local scope. Generic process execution remains truthfully OS-user scoped.
- **#177 — executor-aware readiness:** normal-serving Ready requires the pinned local execution backend to pass the bounded readiness proof. Process/liveness, Local MCP listening, executor readiness and tunnel reachability remain distinct；transport presence alone is not device Ready.
- **#178 / #181 / #183 — paired-device Relay path:** current accepted topology is `ChatGPT -> one account-facing Local Connector Relay -> exact paired device -> device-local Stable Runtime/Local MCP`. Devices have stable opaque `deviceId` plus Online / Last seen / executorReady / Ready and revoke semantics. Relay-owned workspace/process handles preserve account + device + runtime affinity without Governance task/Parent tokens. `connectionEpoch/runtimeId/requestId` fence transport/runtime/request identity；same-runtime reconnect preserves handles, while runtime restart or revoke invalidates stale handles. Duplicate display names are irrelevant and bound-device offline/non-ready never silently fails over.
- **#185 — real two-device dogfood:** **PASS / Parent ACCEPTED / COMPLETE** against `main@6d7540086d08f430e5e64b7abc411c64627a92f1`. A/B typed reads and process sentinels reached the exact physical machines；workspace/process affinity, reconnect continuity, runtime restart fencing, non-ready no-failover, duplicate-name routing, revoke isolation and #176 local filesystem denial were all proven from the ChatGPT-facing Relay path.
- **Boundary:** the device-first North Star is `ChatGPT account -> authorized devices -> per-device local policy -> workspace/resource context -> operation`. The canonical single-device Secure Tunnel path remains available as rollback/migration capacity. The dedicated dogfood Relay/Tunnel path is proven but does **not** itself imply production relay hosting, final OAuth/OIDC binding, default flip, release, or retirement of the canonical path. Remote Desktop Commander remains fallback/bootstrap/control capacity rather than an accepted-relay execution dependency. Generic CLI/batch applications continue to use the existing process capability rather than per-app wrappers absent new evidence.
...
- **Durable Local Governance:** explicit Governance mission / Brain Continuity / Codex sustained-execution 路径的 live control truth；ordinary Direct Local file/workspace capability 以 authorized device endpoint + workspace/resource safety 为授权边界，不要求 Governance mission token。Human-facing conversation names 不替代 Governance authority generation/fencing semantics。
