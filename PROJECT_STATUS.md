...
- **Issue #159 — device-first Local Connector correction (implementation candidate):** Human REPLAN 将 ordinary Direct Local 从 Governance mission authority 解耦。普通 `edit/filesystem/Excel/DOCX/PDF/workspace-effect verify` 由 connected Local Connector/device endpoint + `workspaceId`/resource scope + operation-specific safety 授权，不再公开或要求 `taskId/authorityToken/executionToken`；host trust ceiling、primary/secondary separation、sensitive/canonical-path policy、symlink/junction rejection、MutationOwner、`baseSha256`、atomic/readback 均保持。Governance 保留为 optional orchestration/continuity/acceptance overlay；Codex authorization contract 不在本 issue 改动。该条在 Parent 接受并合并 exact head 前仍是 #159 candidate，而不是新的 release/default 状态。
