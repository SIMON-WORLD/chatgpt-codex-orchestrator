...
test('Issue #124: secondary read grant changes persist and fence old execution claims without widening mutation root', () => {
  const { dataRoot, workspace, otherWorkspace, namespace } = fixture('issue124-grant-fence-');
  const d = createDurableGovernanceService({ dataRoot, namespace });
  const ids = createTask(d, { workspace });

  const first = d.transition({
    taskId: ids.taskId, stepId: ids.stepId, control: 'REVISE',
    secondaryReadGrants: [{ kind: 'root', path: otherWorkspace }],
    authorityToken: ids.parentToken, workspaceRoot: workspace,
  });
  assert.deepEqual(first.secondaryReadGrants, undefined);
  assert.deepEqual(d.status().secondaryReadGrants, [{ kind: 'root', path: otherWorkspace }]);
  assert.deepEqual(d.store.loadTask(ids.taskId).state.secondaryReadGrants, [{ kind: 'root', path: otherWorkspace }]);

  const claim = d.claimExecution({ taskId: ids.taskId, stepId: ids.stepId, workspaceRoot: workspace });
  assert.equal(claim.executionClaim.active, true);

  d.transition({
    taskId: ids.taskId, stepId: ids.stepId, control: 'REVISE',
    secondaryReadGrants: [{ kind: 'file', path: path.join(otherWorkspace, 'narrow.txt') }],
    authorityToken: ids.parentToken, workspaceRoot: workspace,
  });
  assert.throws(
    () => d.authorizeExecution({ taskId: ids.taskId, executionToken: claim.executionToken, workspaceRoot: workspace }),
    (e) => e.code === 'stale_execution_claim',
  );

  assert.throws(
    () => d.authorizeMutation({ taskId: ids.taskId, authorityToken: ids.parentToken, workspaceRoot: otherWorkspace }),
    (e) => e.code === 'workspace_mismatch',
  );
  assert.equal(
    d.authorizeMutation({ taskId: ids.taskId, authorityToken: ids.parentToken, workspaceRoot: workspace }).workspaceRoot,
