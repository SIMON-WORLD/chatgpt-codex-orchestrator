# chatgpt-codex-orchestrator — v0.1.0-alpha.1 (DOGFOOD)

A durable **ChatGPT-command orchestration** loop: ChatGPT = planner/reviewer, Codex = executor. One user goal → ChatGPT plans → Codex executes → result returns → ChatGPT reviews → until DONE/ASK_USER. It persists Task State, keeps one persistent Codex thread, survives a runtime crash, and redacts secrets.

## Architecture

```
ChatGPT Brain
↕ IAB BrainSession (Codex in-app browser)
↕ Brain Context / PacketContextProvider
↕ Structured Protocol + Acceptance/Evidence gate
↕ TaskManager / durable Task State (turn-sliced advanceTask)
↕ Runtime Host (localhost IPC + auth)
↕ persistent Codex worker / thread
```

## Supported (v0.1.0-alpha.1)

- `conversation: 'new'` (default) — new ChatGPT conversation + persistent Codex thread.
- ChatGPT Brain: `TASK` / `REVISE` / `ASK_USER` / `DONE` control loop.
- Structured `acceptance[]` + structured `evidence[]`; Acceptance Gate on DONE (required acceptance must have real `pass` evidence; never from codex exit code alone).
- Durable Task State (schema v1) + turn-sliced `advanceTask` (each unit < node-REPL time cap).
- `resumeTask` / `recovery_required` (crash-safe; stale locks reclaimed; re-binds the same conversation/thread).
- Crash-safe `TaskLock` (owner pid + heartbeat; active owner rejected, dead+stale reclaimed).
- Brain Project Profile (`bindProject` / `getProjectBinding`) — local binding.
- `PacketContextProvider` — bounded, secret-redacted, no whole-repo.
- `doctor` / redaction / safe defaults (no dangerous bypass by default).

## Experimental

- `conversation: 'current'` / `adopt-current`. The implementation is retained (binding frozen at creation + persisted-binding resolver for tab-lost/drift), but it is **not** a stability promise in alpha.1, because `tabs.selected()` / selected-tab identity is unstable across node-REPL invocations in the current Codex Desktop / IAB environment.

## Known limitations

- The Codex worker must run in an ordinary Node process (node-REPL sandbox forbids a descendant codex); the bootstrap layer starts it, but it is a runtime deployment constraint.
- The default user data root (`%LOCALAPPDATA%`) is not writable from this sandbox; the Alpha entry must supply a durable writable root via the resolver (workspace-candidate) or `CHATGPT_ORCHESTRATOR_DATA_ROOT`.
- The local governor passes the bearer token on the codex child argv (redacted in logs/state, not removed from argv).
- Depends on the ChatGPT web DOM; selectors/placeholders may need updates if the UI changes.
- No cross-process auto-recovery beyond `resumeTask`; no concurrent task queue; no cost ledger.

## Future roadmap

- Stabilize `adopt-current` (identity binding via explicit tabId instead of `tabs.selected()`).
- Cloudflare/remote runtime, cost ledger, richer context providers, GUI-free daily entry.

## Dogfood usage

1. `doctor` → preflight checks (all PASS/WARN; FAIL gives actionable reason + chosen dataRoot/source).
2. Start a new task: `start new task` → `TaskService.startTask({ goal, repoDir, conversation: 'new' })`.
3. `status <taskId>` / `resume <taskId>` / `cancel <taskId>`.

User-facing natural-language entry:

> 用 ChatGPT 指挥模式完成这个任务：<goal>

---
---# chatgpt-codex-orchestrator (M1: Reliable Brain Transport)

Reliable, minimal transport so **Codex = local executor** can talk to **ChatGPT = brain/planner/reviewer** entirely through the **Codex in-app browser (iab)**. This is M1 of `chatgpt-codex-orchestrator`; it is **not** the full orchestrator, and works in the ChatGPT-command loop already proven in M0.
...
Expected: `ownedTabIdStable`, `conversationIdStable`, `assistantCountIncrements`,
`eachRoundIsNewReply`, `allNoncesEchoed`, `singleOwnedTabAdded`, `otherTabsUntouched` all `true`.
...
- Not yet: task state manager, planner/reviewer loop, routing, persistence of conversation across Codex restarts (M1 only proves the transport).
...
- IPC: worker binds localhost only; each worker process has a random `token`; requests must carry `auth` (token) + optional `taskId`; mismatches are rejected.
- Secrets: bearer/API keys redacted from logs/RESULT/state; `TaskLog` is structured JSONL with a size cap.
- Execution defaults: `bypassSandbox` is NOT the default; the worker host only enables `--dangerously-bypass-approvals-and-sandbox` when passed `--bypass` (explicit development opt-in). `repoDir` is normalized to an absolute path.
- Compatibility limitation: the node REPL sandbox denies a descendant codex process (`~/.codex/tmp/arg0` Access denied), so the Codex worker must be launched by the environment (not as a node-REPL child). IPC token/taskId are used, but the bearer token is still passed to codex on the child command line (current provider auth), and is redacted from all returned text/logs.

### B5 doctor

`doctorStatic` checks codex.js discoverability, codex version, stateDir writable, repoDir legal, bypass detection. `doctorLiveIo` checks iab availability and ChatGPT composer/login. Returns a list of `{ check, status: PASS|WARN|FAIL, reason }`.

### Limits (Batch B)

- B5 live Alpha E2E (single-entry + process crash + restart) could not be cleanly captured in this run: the node-REPL sandbox prevents a descendant codex, so the worker is launched externally; repeated IPC/codex calls sometimes stalled and reset the node REPL kernel. The crash/restart + resume mechanism (same conversation + same codex thread via `--session`, plus task lock) is implemented and covered by offline tests and the Batch A live resume; a fully-clean live crash/restart run remained flaky.
- The provider bearer token is passed on the codex child command line (inherent to the local governor); it is redacted from returned text, logs and state, but not from the process argv.
