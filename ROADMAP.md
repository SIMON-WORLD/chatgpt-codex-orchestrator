...
| Multi-device Local Connector product path | **CLOSED / ACCEPTED / PROVEN** | Issues #176/#177/#178/#181/#183/#185；device-first per-device policy + executor-aware Ready + account-facing Relay + exact paired-device routing；real two-device dogfood PASS；canonical single-device path retained |
...
## Local Connector recovery lineage — ACCEPTED through #128

This is post-v0.2 accepted stabilization/recovery truth, not a new milestone, default flip or release gate.

- **[#116](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/116) → [#117](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/117) / [#118](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/118) → [#120](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/120):** `COMPOSITE_DESKTOPCOMMANDER_ENGINE` is accepted and proven. Keep the existing custom App / Secure MCP Tunnel / thin Orchestrator façade and unchanged public contracts; delegate bounded commodity read/search/fixed-git execution to exact-pinned DesktopCommander 0.2.51 child MCP over stdio; keep Codex App Server separate. Superseded bespoke direct read/search/git execution fallbacks are deleted. Hosted Remote Desktop Commander, generic public shell/config/write-file, a maintained fork and a second engine are not part of the accepted path.
- **[#119](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/119):** deployed human-facing name is **Local Connector** with description `Connect ChatGPT to authorized files, workspaces, and local execution capabilities on your computer.`; repository/server identity remains `chatgpt-codex-orchestrator`; DesktopCommander remains dependency/provenance only.
- **[#122](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/issues/122):** exact 0.2.51 npm artifact + exact upstream source commit are stored in the project-owned [dependency escrow prerelease](https://github.com/SIMON-WORLD/chatgpt-codex-orchestrator/releases/tag/dependency-escrow-desktop-commander-0.2.51), with repository manifest/pointer [here](dependency-escrow/desktop-commander-0.2.51.json). Cold recovery without live DesktopCommander npm/GitHub source is proven. Escrow is supply continuity only, not release/default policy.
...
## Device-first Local Connector / real-workload adoption — accepted through #185
...
- **#176 / #177:** each device owns explicit local filesystem policy (`selected_roots` or explicit `os_user_scope`) plus executor-aware normal-serving Ready；transport/listener presence alone is not Ready, and Relay cannot widen filesystem scope.
- **#178 / #181 / #183:** accepted multi-device topology is `ChatGPT -> one account-facing Local Connector Relay -> exact paired device -> device-local Stable Runtime/Local MCP`. Routing uses stable opaque `deviceId`; public state includes Online / Last seen / executorReady / Ready and revoke. Relay-owned workspace/process handles carry account + device + runtime affinity without Governance task/Parent tokens；`connectionEpoch/runtimeId/requestId` fence stale transport/runtime/request identity；same-runtime reconnect preserves handles, runtime restart/revoke fences stale handles, and bound-device offline/not-ready never silently fails over.
- **#185:** one Local Connector Relay reached two real computers and passed A/B read/process sentinels, handle affinity, reconnect continuity, runtime restart fencing, non-ready no-failover, duplicate display-name routing, revoke isolation and local filesystem denial. Parent accepted #185 complete against `main@6d7540086d08f430e5e64b7abc411c64627a92f1`.
- **Current operating state:** device-first authority is `ChatGPT account -> authorized devices -> per-device local policy -> workspace/resource context -> operation`. The canonical single-device Secure Tunnel path remains available as rollback/migration capacity；the dedicated Relay dogfood path is proven but does not imply production hosting, final OAuth/OIDC binding, default flip or release. Remote Desktop Commander remains fallback/bootstrap/control capacity rather than a dependency of accepted relay execution. Generic CLI/batch use continues through the existing process capability rather than per-app wrappers absent new evidence.
- **Transport/tool-shape discipline:** retain the canonical Secure MCP Tunnel path；treat relay hosting/auth productionization as a separate Parent decision；do not rename/re-annotate/repackage tools or reintroduce Governance merely to chase product safety admission.
